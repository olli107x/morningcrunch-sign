...
module ReplaceEmailVariables
  ...
  def build_documents_links_text(submitter, sig = nil)
    Rails.application.routes.url_helpers.submissions_preview_url(
      submitter.submission.slug, { sig:, **MorningcrunchSign.default_url_options }.compact
    )
  end
  ...
  def build_submission_link(submission)
    Rails.application.routes.url_helpers.submission_url(submission, **MorningcrunchSign.default_url_options)
  end
